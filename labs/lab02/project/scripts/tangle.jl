...
Примеры:
  julia tangle.jl scripts/sir_ode.jl
  julia tangle.jl scripts/lv_ode.jl
""")
        return
    end
    ...
    # Чистый скрипт (без комментариев) — в подкаталог с именем скрипта
    scripts_out = scriptsdir(script_name)
    mkpath(scripts_out)
    Literate.script(script_path, scripts_out; credit=false)
    println("  ✓ Чистый скрипт: $(scripts_out)/$(script_name).jl")
