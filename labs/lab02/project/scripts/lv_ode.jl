...
# **Цель:** Исследовать динамику взаимодействия двух видов: хищников и жертв.
#
...
## Фильтруем нулевую частоту для лог. масштаба
freq_pos = freq_prey[2:end]
spec_prey_pos = spectrum_prey[2:end]
spec_pred_pos = spectrum_predator[2:end]

plt5 = plot(freq_pos, [spec_prey_pos spec_pred_pos],
    label=[L"Жертвы (x)" L"Хищники (y)"],
    xlabel="Частота",
    ylabel="Амплитуда",
...
scatter!(plt6[3], [x_star], [y_star], color=:black, markersize=5,
    label="(x*, y*)")
plot!(plt6[4], df_lv.t, df_lv.dprey_dt,
    label=L"dx/dt", color=:green, linewidth=1.5,
        title="Скорости изменения", grid=true, legend=:topright)
plot!(plt6[4], df_lv.t, df_lv.dpredator_dt,
    label=L"dy/dt", color=:red, linewidth=1.5)
plot!(plt6[5], freq_pos, spec_prey_pos, label=L"x", color=:green,
    linewidth=1.5,
        title="Спектр жертв", xscale=:log10, yscale=:log10, grid=true)
plot!(plt6[6], df_lv.t, df_lv.prey_pct_change,
    label=L"dx/x", color=:green, linewidth=1.5,
        title="Относительные изменения", legend=:topright)
plot!(plt6[6], df_lv.t, df_lv.predator_pct_change,
    label=L"dy/y", color=:red, linewidth=1.5)
