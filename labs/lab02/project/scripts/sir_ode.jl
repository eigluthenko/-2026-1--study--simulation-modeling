...
# **Цель:** Исследовать трёхпараметрическую модель SIR (Susceptible-Infectious-Recovered).
#
...
# - $\frac{dS}{dt} = -\beta c I S / N$
# - $\frac{dI}{dt} = \beta c I S / N - \gamma I$
# - $\frac{dR}{dt} = \gamma I$
#
# Параметры: $\beta$ — вероятность заражения, $c$ — число контактов, $\gamma$ — скорость выздоровления.
...
# ## 3. График в логарифмическом масштабе (для анализа экспоненциального роста)
...
# ## 6. График Re — эффективного репродуктивного числа
...
# Отметка момента, когда Re становится < 1
...
# ## 7. Компактный график всех кривых в одной панели
